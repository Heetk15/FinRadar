    ...
    stages {
        ...
        stage('Sync Workspace To Target') {
            ...
            steps {
                ...
                sshagent(credentials: ['deploy-host-ssh']) {
                    ...
                    ARCHIVE_NAME="FinRadar-workspace-${BUILD_NUMBER}.tgz"
        ...
        stage('Deploy (Kubernetes)') {
            ...
            steps {
                ...
                sed -i 's/127.0.0.1/host.docker.internal/g; s/localhost/host.docker.internal/g' /tmp/kubeconfig
                export KUBECONFIG=/tmp/kubeconfig
                kubectl create namespace FinRadar-prod --dry-run=client -o yaml | kubectl apply -f -
                kubectl apply -f k8s/ -n FinRadar-prod
                '''
            }
        }
